...
<jsp:include page="../header.jsp"></jsp:include>
	...
	<form id="registForm" action="/sboard/register" method="post">
		...
		<table border=1 style="width: 100%;" class="table table-hover table-bordered">
			<tr>
				...
				<td colspan=2><input type="text" id="title" name="title"> <!-- Trigger/Open The Modal -->
					<button type="button" id="myBtn"class="btn btn-info">검색</button> <!-- The Modal -->
		...
		<div class="sB">
			...
			<input type="button" id="saveBtn"class="btn btn-info " value="저장하기" />
...
<script>
	...
	function getPageList(page){
		...
		$.getJSON("query/"+title+"/"+page,function(data){
			...
			html +="<tr>"
				...
				html += "<td class='movietitle '><input type='button' class='btn btnSelect btn btn-info' value='선택'/></td>"
	...
	$(document).on("click", function(e){
      if($("#myModal").is(e.target) && !$(".modal-content").is(e.target)){
         $("#myModal").css("display", "none");
      }
   });
</script>
